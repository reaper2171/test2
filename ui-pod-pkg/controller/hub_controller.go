...
func AddingRequestToServerQueue(request model.CypressRequestModel, specFile []string, w http.ResponseWriter, requestId string, timestamp string) {
...
func StartCypressTest(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Hub connected")

	var request model.CypressRequestModel
	...
	log.Printf("After Shorting Tags arrangement are : [%v]", request.Spec.Tags)
...
// Rerun code
func StartCypressRerunTest(w http.ResponseWriter, r *http.Request) { //have to take array of testnames in body
	fmt.Println("Hub connected")
	...
	// setting hub-request-map with the reqeust
	config.SetHubRequestMap(requestId, request)

	log.Printf("req.spec.test is : %v", request.Spec.Test)
	log.Printf("Given request is : %v", request)

	//for adding the tests feature file in an array from key value mapping
	var tests []string
	for _, test := range request.Spec.Test {
		tests = append(tests, test.FeatureName)
	}
	...
	// if request.Spec.IsTag { //As there is only one way to pass failed testcases data
...
func ChunkCreation(request model.CypressRequestModel, currentTimeStamp string, w http.ResponseWriter) {
...
func UpdateTestCompletion(response model.CypressResponseModel) {
	...
	if numberOfChunkLeft == 0 {
		...
		// Appending uuid to suite.json using testname and suitename
		hubReq, err := config.GetHubRequestMap(response.RequestId)
		if err != nil {
			log.Printf("error getting request from hub-request-map using requestId to add uuid: %v", err)
			return
		}
		err = utils.AppendToJsonReport(hubReq)
		if err != nil {
			log.Printf("error appending parentUUID to suite.json file: %v", err)
			return
		}

		// running python script for failedcases.json and passedcases.json
		err = utils.RunPythonScirpt()
		if err != nil {
			log.Printf("error running the python script: %v", err)
			return
		}

		err = utils.ProcessingReports()
		if err != nil {
			log.Printf("error running the python script: %v", err)
			return
		}

		// calling function for hitting upload api
		zipFilePath := "./final.zip"
		targetURL := "http://192.168.6.149:1234/api/upload"
		params := map[string]string{
			"runtype":     "ui",
			"environment": "HOTFIX",
			"jobName":     "TESTCASE-UI-Regression-HotFix-Job5-Testing",
			"rerun":       "rerun",
		}

		err = utils.UploadFile(zipFilePath, targetURL, params)
		if err != nil {
			log.Printf("error in hitting upload API: %v", err)
		}
...
func EnvironmentTagsPressence(request model.CypressRequestModel) bool {
...
// env check for rerun
func EnvironmentTagsPressenceForRerun(request model.CypressRequestModel) bool {
	...
	for _, test := range request.Spec.Test {
		currTagList = append(currTagList, test.FeatureName)
	}
...
// for handling pod redis calls
func UpdateRedisFromPod(w http.ResponseWriter, r *http.Request) {
	var request model.PodRedisModel
	err := json.NewDecoder(r.Body).Decode(&request)
	if err != nil {
		log.Printf("can't decode request json: [%v]", err)
		http.Error(w, "Invalid request payloa for pod redis model", http.StatusInternalServerError)
		return
	}

	if request.RequestType == "update_server_busy" {
		config.SetServerAsBusy(request.ServerName, utils.REDIS_MAP_NAME)
	} else if request.RequestType == "update_server_free" {
		config.UpdateServer(request.ServerName, true)
	} else if request.RequestType == "init" {
		config.InitServerReportMap(request.ServerName)
	} else {
		log.Printf("invalid request type passed in the pod redis request")
		return
	}
}

// function to call springboot application to clone/pull delete test repo in ec2
func GetTestRepo(repo string) int {
	respClone, err := http.Get("http://localhost:7000/cloning/" + repo)
	if err != nil {
		log.Printf("error in the cloning request to the jgit app: %v", err)
		return 0
	}
	defer respClone.Body.Close()

	if respClone.StatusCode == http.StatusOK {
		bodyBytes, err := io.ReadAll(respClone.Body)
		if err != nil {
			log.Printf("error reading cloning response from jgit application: %v", err)
			return 0
		}
		bodyString := string(bodyBytes)
		if bodyString == "Cloned successfully" {
			log.Printf("Automation repo cloned successfully")
			return 1
		} else if bodyString == "Error in cloning the repo" {
			log.Printf("Error in cloning the repo")
			return 0
		} else if bodyString == "Repo ALready Exists" {
			log.Printf("Repo ALready Exists")
			respPull, err := http.Get("http://localhost:7000/pulling/" + repo)
			if err != nil {
				log.Printf("error in the pull request to the jgit app: %v", err)
				return 0
			}
			defer respPull.Body.Close()

			if respPull.StatusCode == http.StatusOK {
				bodyBytesPull, err := io.ReadAll(respPull.Body)
				if err != nil {
					log.Printf("error reading response from jgit application: %v", err)
					return 0
				}
				bodyStringPull := string(bodyBytesPull)
				if bodyStringPull == "Pull was succesful" {
					log.Printf("Repo pull was successful")
					return 1
				} else if bodyStringPull == "Git cloned because there is no previous cloned repo" {
					log.Printf("No cloned repo exists, repo cloned successfully")
					return 1
				} else if bodyStringPull == "Error because of Authentication" {
					log.Printf("Error in pulling the test repo")
					return 0
				} else if bodyStringPull == "Error because of API" {
					log.Printf("Error in pulling the test repo because of API error in the jgit app")
					return 0
				} else if bodyStringPull == "Error due to conflict issue" {
					log.Printf("Error in pulling the test repo because of conflicts")
					return 0
				}
			}
		}
