...
// function to search postman collection to run based on the suitename
func CreatePostmanCollection(testDetails []model.TestData, searchDir string) error {

	// checking whether there is atleast one suitename-testname
	if len(testDetails) == 0 {
		log.Printf("empty suitename-testname list")
		return fmt.Errorf("at least one suitename-testname must be provided")
	}

	// finding the suite chunk which has to be run for each unique suitename testname combination
	var outputFile string
	for i := 0; i < len(testDetails); i++ {
		outputFile = ""
		utils.SearchCollectionUsingBash(testDetails[i], searchDir, outputFile)
		testDetails[i].Collection = outputFile
	}

	// Creating two collection files for chunks, sorting the collection chunk and writing them to collections
	// according to their size
	first, err := os.Create("firstCollection.json")
	if err != nil {
		log.Printf("Error creating first collection file: %v", err)
		return err
	}
	second, err := os.Create("secondCollection.json")
	if err != nil {
		log.Printf("Error creating second collection file: %v", err)
		return err
	}

	sortedTestDetails := testDetails
	sort.Slice(sortedTestDetails, fu)

	var newCollection string = "["
	for _, val := range testDetails {
		newCollection = newCollection + val.Collection + ","
	}
	// newCollection = newCollection + "]"
	// Collection = newCollection.JSONMarshal()
	return nil
}
